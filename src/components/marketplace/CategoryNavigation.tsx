...
const CategoryNavigation = ({
  ...
  // Mock de subcategorias (apenas exibição)
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        setError(t('nav.errors.categoriesLoadFail'));
        setCategories([
  ...
  const handleCategorySelect = async (categoryId: number | string, categoryName?: string) => {
    if (onCategorySelect) {
      try {
        ...
        console.error('Failed to filter products by category');
      }
    }
  };
    ...
    return (
      <div className={`bg-black-800 rounded-lg border border-gem-purple/20 p-4 ${className}`}>
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-xl font-semibold text-white">{t('nav.title')}</h3>
          {selectedCategoryId && (
            <button
        ...
        {selectedCategoryName && (
          <div className="mb-4 px-3 py-2 bg-gem-purple/10 border border-gem-purple/30 rounded-md">
            <p className="text-sm text-white/80">
              {t('nav.filteringBy')}{' '}
              <span className="text-gem-pink font-medium">{selectedCategoryName}</span>
            </p>
          </div>
        )}
